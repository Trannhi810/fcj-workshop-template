...
1. **Giai đoạn 1. Thiết lập môi trường & Container**: Cấu hình IAM/AWS CLI, xây dựng Docker/Playwright runner (Dockerfile, entrypoint.js,playwright.config.js).
2. **Giai đoạn 2. Luồng sự kiện & Coordinator**: Thiết lập SQS + DLQ và Lambda Coordinator gọi ECS RunTask.
3. **Giai đoạn 3. Lưu trữ & Giám sát**: S3 (frontend + báo cáo), log group và alarm CloudWatch, bảng DynamoDB lưu lịch sử và audit log.
4. **Giai đoạn 4. Tóm tắt bằng AI**: Lambda hậu kỳ tích hợp Secrets Manager cho khóa OpenAI API và cơ chế fallback dạng circuit-breaker.
5. **Giai đoạn 5. Dashboard & Phân quyền**: Lưu trữ tĩnh CloudFront + S3, Cognito user pool, Lambda Authorizer và giao diện 3 vai trò (Admin / QA-Tester / Developer).
6. **Giai đoạn 6. Tăng cường bảo mật**: Chính sách IAM theo nguyên tắc đặc quyền tối thiểu, VPC Endpoints và hai phạm vi WAF (CloudFront và Regional).
7. **Giai đoạn 7. Kiểm thử tích hợp & Demo**: Chạy thử end-to-end trên một website demo tự dựng (tránh vấn đề chính sách chống bot của bên thứ ba), kiểm chứng toàn bộ pipeline, chuẩn bị báo cáo.
...
- **Tháng 1**: Khởi tạo hạ tầng, setup môi trường AWS, cấu hình IAM Roles và xây dựng core Playwright chạy trên Docker.
- **Tháng 2**: Triển khai kiến trúc Serverless với API Gateway, SQS, EventBridge và Fargate. Phát triển Backend logic bằng Lambda. Thiết lập S3 lưu báo cáo.
- **Tháng 3**: Tích hợp AI Post-processing, cấu hình Cognito và phát triển Frontend Dashboard. Hoàn thiện bảo mật VPC, WAF và tiến hành test End-to-End toàn hệ thống.

### 6. Ước Tính Ngân Sách (Budget Estimation)
| Dịch vụ AWS | Cấu hình chính (AWS Calculator) | Chi phí/tháng (USD) |
|---|---|---|
...
*Ma trận rủi ro*
- **Tác vụ Fargate chạy vượt quá thời gian dự kiến / timeout**: Mức độ ảnh hưởng Trung bình, Xác suất Trung bình.
- **AI API bên ngoài (OpenAI) không khả dụng hoặc bị giới hạn tốc độ**: Mức độ ảnh hưởng Thấp, Xác suất Trung bình.
- **Vai trò IAM được cấp quá nhiều quyền trong quá trình phát triển**: Mức độ ảnh hưởng Cao, Xác suất Trung bình.
- **Tin nhắn tồn đọng trong DLQ mà không được cảnh báo**: Mức độ ảnh hưởng Trung bình, Xác suất Thấp.
- **Chi phí AWS vượt dự kiến**: Mức độ ảnh hưởng Trung bình, Xác suất Thấp.
- **Độ trễ khi QA kích hoạt test đột xuất ngoài khung giờ NAT Gateway đã lên lịch**: Mức độ ảnh hưởng Trung bình, Xác suất Trung bình.
