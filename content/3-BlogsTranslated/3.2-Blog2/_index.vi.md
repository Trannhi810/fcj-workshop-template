...
### 1. Đọc Hiểu Giao Diện (UI Contextual Awareness)

Thay vì chỉ nhìn vào các dòng code HTML khô khan, Amazon Bedrock giúp Rapise *"nhìn"* và hiểu giao diện giống như một con người — biết đâu là nút bấm, đâu là ô nhập liệu dựa trên ngữ cảnh.

### 2. Tự Động Chữa Lành (Self-Healing Scripts)

Khi ứng dụng cập nhật và làm thay đổi cấu trúc UI, AI sẽ tự động phân tích và tìm ra phần tử thay thế phù hợp nhất để tiếp tục chạy bài test **mà không cần kỹ sư phải vào sửa code thủ công**.

### 3. Tự Động Sinh Kịch Bản (AI-Driven Generation)

Chỉ cần mô tả bằng ngôn ngữ tự nhiên (ví dụ: *"Kiểm tra tính năng thêm sản phẩm vào giỏ hàng"*), AI kết hợp với Rapise sẽ tự động đề xuất và xây dựng các bước test tương ứng.
...
Tuy nhiên, vì đây là giải pháp thuộc hệ sinh thái đóng (mô hình thương mại có phí của Rapise và hạ tầng Cloud của AWS), nó sẽ phù hợp nhất với **các tổ chức lớn đã có sẵn hạ tầng AWS**, thay vì các dự án startup nhỏ ưu tiên công cụ mã nguồn mở (Open-source).

Nhưng nhìn chung, kỷ nguyên **"AI-powered Test Automation"** đã thực sự bắt đầu và sẽ sớm thay đổi hoàn toàn cách chúng ta làm QA phần mềm.

Chi tiết về cấu hình và các bước thiết lập kỹ thuật, bạn có thể tham khảo thêm tại bài viết gốc:
